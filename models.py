...
class Post(db.Model):
    ...
    edit_time: Mapped[int] = mapped_column(default = 0) # Time when post was last edited only displayed if it was edited
...
class Comment(db.Model):
    ...
    username: Mapped[str]
    content: Mapped[str]
    time: Mapped[int]  # epoch time -- when comment was originally posted
    edit_time: Mapped[int] = mapped_column(default = 0) # If never edited
